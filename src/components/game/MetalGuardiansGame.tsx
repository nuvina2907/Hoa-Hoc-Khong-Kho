...
export default function MetalGuardiansGame() {
  ...
  const spawnExplosion = (x: number, y: number, colorClass: string) => {
    const colorHex = colorClass.includes('rose') ? '#f43f5e' : 
                     ...
                     colorClass.includes('emerald') ? '#10b981' : '#fbbf24';
  ...
  const handleShoot = useCallback((type: EnemyType | 'ultimate') => {
    ...
    setTimeout(() => setBeam(b => ({ ...b, active: false })), 200);
    ...
    if (closestIdx !== -1) {
      ...
      if (target.type === type) {
        ...
        particlesRef.current.push({
          ...
          y: target.y,
          vx: 0, vy: -30,
          color: colorClass.includes('rose') ? '#f43f5e' : colorClass.includes('cyan') ? '#22d3ee' : colorClass.includes('blue') ? '#3b82f6' : '#10b981',
          text: '+10',
          life: 1, maxLife: 1, type: 'text'
        });
  ...
  const renderBeam = () => {
    ...
    if (beam.type === 'nhiet') {
      return (
        <div 
          className="absolute right-[15%] h-8 rounded-l-full origin-right z-20 flex items-center"
          style={{ top: `${beam.y}%`, width: '85%', transform: 'translateY(-50%)' }}
        >
          <div className="w-full h-full bg-gradient-to-l from-rose-600 via-orange-500 to-transparent opacity-80 blur-sm absolute"></div>
          <div className="w-full h-2 bg-white rounded-full shadow-[0_0_20px_#f97316] relative z-10"></div>
        </div>
      );
    }
    if (beam.type === 'dien') {
      return (
        <svg 
          className="absolute right-[15%] h-20 origin-right z-20"
          style={{ top: `${beam.y}%`, width: '85%', transform: 'translateY(-50%)' }}
          preserveAspectRatio="none"
        >
          <path 
            d={`M 100% 50% L 75% ${30 + Math.random()*40}% L 50% ${30 + Math.random()*40}% L 25% ${30 + Math.random()*40}% L 0% 50%`} 
    ...
    if (beam.type === 'thuy') {
      return (
        <div 
          className="absolute right-[15%] h-[200px] w-[200px] border-[8px] border-blue-400 rounded-full z-20 animate-[water-ripple_0.6s_ease-out_forwards]"
          style={{ top: '50%', transform: 'translate(50%, -50%)', boxShadow: '0 0 40px #3b82f6, inset 0 0 40px #3b82f6' }}
        />
      );
    }
    if (beam.type === 'recycle') {
      return (
        <div 
          className="absolute right-[15%] h-4 rounded-l-full origin-right shadow-[0_0_20px_#10b981] bg-gradient-to-l from-emerald-400 to-transparent z-20"
          style={{ top: `${beam.y}%`, width: '85%', transform: 'translateY(-50%)', opacity: 0.9 }}
        />
      );
    }
    return null;
    ...
    <div className="max-w-6xl mx-auto space-y-6">
      ...
      >
        ...
        <div className="absolute bottom-6 left-6 flex gap-3 z-30">
          ...
          <WeaponKey k="SPACE" label="TÁI CHẾ" color="emerald" icon={<Recycle className="w-6 h-6"/>} />
            ...
            >
              <div className="neumorphic-panel border border-slate-700 p-10 rounded-[2rem] max-w-lg text-center relative overflow-hidden">
                ...
                <div className="space-y-4 text-left bg-slate-950/50 p-6 rounded-2xl mb-8 border border-slate-800/50 shadow-inner">
                  ...
                  <div className="flex items-center gap-4 text-sm text-slate-200"><span className="w-16 h-8 bg-slate-800 rounded-lg flex items-center justify-center font-black text-emerald-400 text-xs shadow-md">SPACE</span> Tái Chế ➔ Thu gom phế liệu</div>
